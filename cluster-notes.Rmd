...
* Multiple versions of SLiM are available on the cluster. [This page](https://biohpc.cornell.edu/lab/userguide.aspx?a=software&i=411#c) provides the commands for 5 different SLiM versions.

* To vary parameters in SLiM, parse SLiM output, and save your desired summary statistics, you can follow Sam's pipeline:

1. Copy and paste Sam's `defineCfgParam` function into your SLiM script (see [distant_site_pan_TA.slim](slim-example/distant_site_pan_TA.slim)):
```{r}
knitr::include_graphics("figures/defineCfgParam.png")
```
In your SLiM `initialize()` callback, define any parameter that you want to vary with `defineCfgParam("PARAMETER_NAME", DEFAULT_VALUE)`. For example, if I wanted to vary the germline resistance rate in a gene drive model but have a default value of 0.9, I'd do `defineCfgParam("GERMLINE_RESISTANCE_RATE", 0.9)`.
